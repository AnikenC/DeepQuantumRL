...
class EnvParams:
    ...
    window_length: Optional[int] = 7
    kernel: Optional[chex.Array] = jnp.ones(window_length) / window_length
    gauss_mean: Optional[int] = 0.0
    gauss_std: Optional[int] = 1.0
    small_window: Optional[chex.Array] = jnp.linspace(-3.0, 3.0, window_length)
    gauss_kernel: Optional[chex.Array] = 1 / (jnp.sqrt(2 * jnp.pi) * gauss_std) * jnp.exp(-(small_window - gauss_mean)**2 / (2 * gauss_std**2))
...
class BatchedPhotonLangevinReadoutEnv(SingleStepEnvironment):
    ...
    def __init__(
        ...
        self.res_amp_scaling = 0.5 * jnp.sqrt(rough_max_photons * (self._kappa**2 + self._chi**2))
        ...
        self.freqs_shifted = jnp.fft.fftshift(jnp.fft.fftfreq(n=len(self.ts_action), d=self._t1/len(self.ts_action)) * self._tau)
    ...
    def step_env(
        ...
        reward, updated_state_array = self.batched_reward_and_state(
            batched_results.astype(self.float_dtype),
        )
    ...
    def get_bandwidth(self, batched_drive: chex.Array):
        ...
        indices = jnp.where(fft_shifted > params.bandwidth_threshold * jnp.max(fft_shifted), size=params.num_actions)[0]
        min_index = indices[0]
        max_index = jnp.max(indices)
        bandwidth = jnp.array(
            [self.freqs_shifted[max_index] - self.freqs_shifted[min_index]], dtype=self.float_dtype
        )
        return bandwidth
    
    def extract_values(self, b_results: chex.Array):
        ...
        b_decay_g = -(self._gamma_I + self._photon_gamma * b_photon_g[:,1:].T) * self.dt
        ...
        b_photons_combined = jnp.concatenate(
            (b_photon_g.reshape(self._batchsize, -1, 1), b_photon_e.reshape(self._batchsize, -1, 1)),
            axis=-1
        )
        ...
        min_photons_ind = jnp.argmin(b_higher_photons[:,self.ind:], axis=-1) + self.ind
        min_photons_times = self.ts_sim[min_photons_ind]
        b_higher_photons += self.photon_limit * jnp.heaviside(self.ts_sim.reshape(1, -1) - min_photons_times.reshape(-1, 1), 0.0)
        
        closest_time_to_reset_ind = jnp.argmin(jnp.abs(b_higher_photons[:, self.ind:] - self._ideal_photon), axis=-1) + self.ind
        pulse_end_times = self.ts_sim[closest_time_to_reset_ind]
        pulse_reset_photons = b_higher_photons[jnp.arange(self._batchsize), closest_time_to_reset_ind]
        photon_reset_time = pulse_end_times + self._tau * jnp.log(pulse_reset_photons / self._ideal_photon)
        photon_reset_time = jnp.clip(photon_reset_time, a_min=pulse_end_times)

        b_pf *= jnp.heaviside(photon_reset_time.reshape(-1, 1) - self.ts_sim.reshape(1, -1), 1.0)
        max_pf = jnp.max(b_pf, axis=-1)

        return max_pf, max_photons, photon_reset_time, b_pf, b_higher_photons
    ...
    def calc_reward_and_state(
        ...
        max_pf, max_photons, photon_reset_time, b_pf, b_higher_photons = self.batched_extract_values(batched_results)
        ...
        batched_reward = (
            ...
            - self.photon_penalty * jax.nn.relu((max_photons / self.actual_max_photons - 1.0))
        )
