...
from algos.rl_wrappers import VecEnv
...
def PPO_make_train(config):
    ...
    def train(
        ...
        network = CombinedActorCritic(
            env.action_space(env_params).shape[0], activation=config["ACTIVATION"], layer_size=config["LAYER_SIZE"]
        )
        ...
        def _update_step(runner_state, unused):
            ...
            if config.get("DEBUG"):
                def return_readout_stats(global_updatestep, info):
                    ...
                    jax.debug.print("mean batch reward: {reward}", reward=info["mean batch reward"])
                    jax.debug.print("mean batch pF: {pF}", pF=info["mean batch pF"])
                    jax.debug.print("mean batch photon: {photon}", photon=info["mean batch photon"])
                    jax.debug.print("mean batch photon time: {time}", time=info["mean batch photon time"])

                    jax.debug.print("max reward obtained: {reward}", reward=info["max reward obtained"])
                    jax.debug.print("pF at max: {pF}", pF=info["pF at max"])
                    jax.debug.print("photon at max: {photon}", photon=info["photon at max"])
                    jax.debug.print("photon time of max: {time}", time=info["photon time of max"])
                def pass_stats(global_updatestep, info):
                    pass
                def return_action(global_updatestep, info):
                    jax.debug.print("action of max={action_of_max}", action_of_max=info["action of max"])
                jax.lax.cond(global_updatestep % config["PRINT_RATE"] == 0, return_readout_stats, pass_stats, global_updatestep, metric)
                if config.get("DEBUG_ACTION"):
                    jax.lax.cond(global_updatestep % config["ACTION_PRINT_RATE"] == 0, return_action, pass_stats, global_updatestep, metric)
